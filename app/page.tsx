...
export default async function Home() {
  ...
  const githubEvents = await getLatestGitHubEvents("BVChandrahaas", 5);

  return (
    <>
      <Nav />
      <main className="mx-auto max-w-6xl px-6 py-10 flex-1 w-full grid grid-cols-1 xl:grid-cols-12 gap-10">
        {/* Main Content Column */}
        <div className="xl:col-span-8">
          ...
          <section className="mb-14">
            ...
            <div className="flex gap-2 text-sm">
              <a
                href="/resume.pdf"
                className="border border-border-strong rounded-md px-3 py-1.5 text-text-secondary hover:text-text-primary transition-colors"
              >
                Resume
              </a>
              <a
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {posts.map((post) => (
                  <Link
                    key={post.slug}
                    href={`/blog/${post.slug}`}
                    className="bg-surface-2 border border-border rounded-xl p-3 block hover:border-border-strong transition-colors"
                  >
